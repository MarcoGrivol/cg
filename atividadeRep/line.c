...
int DrawLine(point * p1, point * p2, window * win, viewport * port, bufferdevice * dev, int color) {
  ...
  if (pd1->x > pd2->x) {  // O swap entre os pontos pode ser necessário. Por quê?
    aux = pd1->x;
    pd1->x = pd2->x;
    pd2->x = aux;
    aux = pd1->y;
...
int RasterFill(bufferdevice * dev, viewport * port, int color) {
  ...
  // dev é a variável que representa o dispositivo físico de saída
  // port é a variável que representa a viewport onde o algoritmo irá preencher o conteúdo
  // color define a cor do preenchimento (representa um índice da lookup table)
